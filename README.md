...
- **规则注入**：`coding-standards/registry.yaml` 中的规则（含 `prompt_hint`、文档路径、`deprecated`）按语言+domain 自动注入到 prompt，且可通过工具读取 Markdown 规则文档。
- **并行与限速**：一个 diff 内的文件审查并行进行；支持通过环境变量配置 QPS 限制。
- **可配置域/黑名单**：通过 profile YAML 为不同仓库指定 domains、文件黑名单、basename 黑名单，提供默认兜底配置。
- **报告生成**：LangGraph 末端节点生成 Markdown 报告（概述 + 带 rule_id 的问题 + 通用问题），并写入 `code_review_report.md`。
...
## 规则与 prompt 注入
- 规则注册表：`coding-standards/registry.yaml`
  - `deprecated: true` 的规则不会进入模型输入。
  - `prompt_hint`、`title`、`severity`、`doc_path` 会在标签 prompt 中展示；若有 `doc_path`，agent 可调用工具读取全文。
- 规则加载：`cr_agent/rules/loader.py` 解析后生成 `RulesCatalog`；`by_language_domain` 用于针对语言+domain 的精确注入。
- 规则工具：`tools/standard_tools.py` 提供 `code_standard_doc(rule_id)` 读取 Markdown 文档。
